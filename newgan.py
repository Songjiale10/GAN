...
dataset = torchvision.datasets.MNIST('data',
                                     ...
                                     download=False)
dataloader = data.DataLoader(dataset, batch_size=64, shuffle=True) #每一次训练epoch，是从DataLoader中获取一个batch_size大小的数据
...
def generate_and_save_images(model, epoch, label_input, noise_input):
    ...
    for i in range(predictions.shape[0]):
        ...
        plt.axis("off")
    plt.savefig(os.path.join("output_img", 'img_at_epoch_{:04d}.png'.format(epoch)))
    plt.show()
